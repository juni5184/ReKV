...
class Abstract_ReKV:
    ...
    def __init__(self, processor, n_frame_tokens, init_prompt_ids, n_local, retrieve_size, chunk_size, sample_fps=None):
    ...
    def _encode_video_chunk(self, video_chunk, is_first=False):
        if is_first: self.encode_init_prompt()
    ...
    def encode_video(self, video, encode_chunk_size=64): # video: (Nv, H, W, 3)
        ...
        for chunk_st in range(0, num_frames, encode_chunk_size): # encode chunk by chunk
            chunk_ed = chunk_st + encode_chunk_size
            chunk_video = video[chunk_st : chunk_ed]
            self._encode_video_chunk(chunk_video, is_first=(chunk_st==0))
    ...
    @property
    def rotary_emb(self):
        return self.language_model.position_bias