...
def eval_qaego4d(args):
    num_chunks = args.num_chunks
    save_dir = f"/workspace/ReKV/results/{args.model}/qaego4d/{args.retrieve_size}-{args.sample_fps}"
    os.makedirs(save_dir, exist_ok=True)
    solver = "rekv_offline_vqa"
    if not args.only_eval:
        # QA
...
def eval_egoschema(args):
    num_chunks = args.num_chunks
    save_dir = f"/workspace/ReKV/results/{args.model}/egoschema/{args.retrieve_size}-{args.sample_fps}"
    os.makedirs(save_dir, exist_ok=True)
    solver = "rekv_offline_vqa"
    if not args.only_eval:
        # QA
...
def eval_activitynet_qa(args):
    num_chunks = args.num_chunks
    save_dir = f"/workspace/ReKV/results/{args.model}/activitynet_qa/{args.retrieve_size}-{args.sample_fps}"
    os.makedirs(save_dir, exist_ok=True)
    solver = "rekv_offline_vqa"
    if not args.only_eval:
        # QA
...
def eval_rvs_movie(args):
    num_chunks = args.num_chunks
    save_dir = f"/workspace/ReKV/results/{args.model}/rvs_movie/{args.retrieve_size}-{args.sample_fps}"
    os.makedirs(save_dir, exist_ok=True)
    solver = "rekv_stream_vqa"
    if not args.only_eval:
        # QA
...
def eval_cgbench(args):
    num_chunks = args.num_chunks
    save_dir = f"/workspace/ReKV/results/{args.model}/cgbench/{args.retrieve_size}-{args.sample_fps}"
    os.makedirs(save_dir, exist_ok=True)
    solver = "rekv_offline_vqa"
    if not args.only_eval:
        # QA
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="llava_ov_7b", choices=['llava_ov_0.5b', 'llava_ov_7b', 'llava_ov_72b', 'video_llava_7b', 'longva_7b', 'qwen2_5_vl_3b'])
